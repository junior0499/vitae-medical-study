...
export function CaseWorkspace() {
  ...
  return <div className="cases-page"><header className="cases-hero"><div><span className="eyebrow"><i /> Recommendation 16 · Progressive case</span><h1>Reason through the case.<br />One reveal at a time.</h1><p>This first case applies only the two live cardiovascular foundation lessons. Disease labels, investigations, and treatment remain locked until approved sources are available.</p><a className="primary-button primary-button--dark" href="/reasoning-ladder">Open six-stage reasoning ladder <span>→</span></a></div><div><span>Current case</span><strong>{clinicalCase.title}</strong><p>{clinicalCase.sourceScope}</p><small>{attempts} saved {attempts === 1 ? "attempt" : "attempts"}</small></div></header>
